...
function clean_image_style($vars){
	if($vars['style_name'] == 'fullscreen' || $vars['style_name'] == 'more_imgs')
	{
		...
		$return = "<div class='slide slide-image'><div class='img-container'><div class=\"zoom-placeholder img-loading $backed\" data-small-src=\"$small_path\" data-small-src-mobile=\"$mobile_path\" data-src-zoom=\"$path\"  data-src-zoom-mobile=\"$mobile_zoom\" style=\"background-image:url('$loading') $background\">";
		$return .=  l('', $style_path, array('html' => 'true', 'attributes' => array(
																		'class' => 'link-to-img', 
																		'width' => '100%',
																		'alt' => $alt, 
																		'title' => $title,
																		'data-src-mobile' => $mobile_path
																		)) );
		$return .= '</div></div></div>';
		return $return;
